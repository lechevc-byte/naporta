    ...
    <>
      ...
      <section className="bg-gradient-to-b from-green-50 to-white">
        <div className="max-w-6xl mx-auto px-4 py-16 sm:py-24 flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
          ...
          {/* Category cards — 3x3, clickable, filter catalogue */}
          <div className="grid grid-cols-3 gap-3 flex-shrink-0">
            {CATEGORIES.map((cat) => (
              <button
                key={cat.name}
                onClick={() => selectCategory(cat.filter)}
              ...
              >
                ...
                <span className="text-[10px] font-bold text-gray-700 mt-1.5 text-center leading-tight px-1">
                  {cat.name}
                </span>
              </button>
            ))}
      ...
      <main ref={catalogRef} id="catalogo" className="max-w-6xl mx-auto px-4 py-12">
        ...
        <p className="text-sm text-gray-400 mb-6">
          ...
          {category !== 'Todos' && (
            <button onClick={() => setCategory('Todos')} className="text-green-600 hover:underline">
              Ver todos
            </button>
          )}
        </p>
        ...
        {/* Category pills — synced with hero */}
        <div className="flex gap-2 overflow-x-auto pb-6 scrollbar-hide">
          {CATEGORIES.map((cat) => {
            const count = cat.filter === 'Todos'
              ? products.length
              : products.filter(p => p.category === cat.filter).length
            ...
            return (
              ...
              >
                {cat.name}
                <span className={`ml-1.5 text-xs ${category === cat.filter ? 'text-green-200' : 'text-gray-400'}`}>
                  {count}
                </span>
              </button>
            )
